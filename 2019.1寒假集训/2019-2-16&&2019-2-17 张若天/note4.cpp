...
f[i][j] = max(f[i-1][j-v[i]*k] + w[i]*k)  0<= k <= a[i] 
...
f[i][j]  飞了i次 高度为j，  最少点击次数


f[i][j] <- f[i-1][j-up[i-1]*k] + k
O(n^3)


1) 只点1次
f[i][j] <- f[i-1][j-up[i-1]] + 1
2) 多点1次 
f[i][j] <- f[i][j-up[i-1]] + 1
O(n^2)
